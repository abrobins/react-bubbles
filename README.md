...
To create authentication in an app.
...
A web server is a computer that's physically connected to the internet and hosts pages or apps that are served online when requests are made to a web server software. The program that runs on this machine is called the web server which serves pages when a request is made.
